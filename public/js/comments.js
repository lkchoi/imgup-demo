...
function commentCreated(response)
{
	...
	var delete_button_id = 'button[delete-comment-id='+response.comment.id+']';
	...
	$(delete_button_id).click(function(e)
	{
		...
		deleteComment($(this).attr('delete-comment-id'));
	});
...
function commentDeleted(response)
{
	...
	$('button[data-comment-id='+response.comment_id+']').parent().parent().parent().remove();
}
