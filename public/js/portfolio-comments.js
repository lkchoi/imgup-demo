...
function createComment() 
{
	...
	$.ajax({
		...
		success: function(response) {
			...
			var commentcounter = parseInt($('#commentcounter').text()) + 1;
			$('#commentcounter').empty();
			$('#commentcounter').append(commentcounter);
		}
	})
}
...
function countComments(response)
{
...
function commentsInit(comment_page)
{
  $.ajax({
    method: 'GET',
    url: '/comments?commentable_type=Portfolio&commentable_id='+portfolio_id+'&page='+comment_page,
    beforeSend: function() {
      $('#loadmorecomments').hide();
    },
    success: function (response) {
